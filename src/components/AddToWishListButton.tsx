...
export default function AddToWishListButton({ product, className }: { product: Product, className?: string }) {
    ...
    return (
        <div className={cn('absolute top-2 right-2 z-10', className)}>
            <button onClick={handleFavorite} className={`p-2.5 rounded-full hover:bg-shop_dark_green/80 hover:text-white transition-all duration-300 ease-in-out  ${existingProduct ? "bg-shop_dark_green/80 text-white" : "bg-lightColor/10"}`}>
                <Heart size={15} />
            </button>
        </div>
    );
