        ...
        <div className="w-full bg-white p-5">
            ...
            <RadioGroup className="mt-2 space-y-1" value={selectedPrice || ""}>
                {priceArray.map((price, index) => (
                    <div key={index} onClick={() => setSelectedPrice(price.value)} className="flex items-center space-x-2 cursor-pointer">
                        <RadioGroupItem
                            value={price?.value}
                            id={price?.value}
            ...
            {selectedPrice && (
                <button onClick={() => setSelectedPrice(null)} className="text-sm font-medium mt-2 underline underline-offset-2 decoration-[1px] hover:text-shop_dark_green transition-all duration-300 ease-in-out">
                    Reset selection
                </button>
            )}
        </div>
