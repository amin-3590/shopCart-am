                                    ...
                                    <tr key={product._id} className="border-b">
                                        <td className="px-2 py-4 flex items-center gap-2">
                                            <X
                                                ...
                                                size={18}
                                                className="hover:text-red-600 hover:cursor-pointer transition-all duration-300 ease-in-out"
                                            />
                                            {product.images && (
                                                <Link href={`/product/${product.slug?.current}`} className="border rounded-md group hidden md:inline-flex">
                                                    <Image
                                                        src={urlFor(product.images[0]).url()}
                                                        alt={"product image"}
                                                        width={80}
                                                        height={80}
                                                        className="rounded-md group-hover:scale-105 transition-all duration-300 ease-in-out h-20 w-20 object-contain"
                                                        unoptimized
                                                    />
                                                </Link>
                                            )}
                                        ...
                                        <td className={`p-2 w-24 ${(product.stock as number) > 0 ? "text-green-600" : "text-red-600"} font-medium text-sm hidden md:table-cell`}>
